      ...
      <div className="flex flex-col gap-3">
        {users.map((u) => (
          ...
          >
            ...
            <span
              className={`rounded-full px-3.5 py-1.25 text-[.78rem] font-bold capitalize ${roleBadgeClass(u.role)}`}
            >
              {u.role}
            </span>
            {u.role === "admin" ? (
              <span className="text-[.8rem] text-text-secondary">
                Admin role can't be changed here
              </span>
            ) : (
              <select
