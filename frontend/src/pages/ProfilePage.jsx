...
export default function ProfilePage() {
  ...
  const [form, setForm] = useState({ name: user?.name || '', avatar: user?.avatar || '', description: user?.description || '' });
  ...
  const startEditing = () => {
    setForm({ name: user.name || '', avatar: user.avatar || '', description: user.description || '' });
    setEditing(true);
  };
  ...
  return (
    <div>
      ...
      {editing ? (
        <form onSubmit={handleSave} className="mb-10 flex flex-col gap-4 rounded-[14px] border border-border-strong p-6.5">
          <h2 className="mb-1 font-display text-[1.3rem] text-text">Edit Profile</h2>
          <input className={inputClass} placeholder="Name" required value={form.name} onChange={setField('name')} />
          <input className={inputClass} placeholder="Avatar image URL" value={form.avatar} onChange={setField('avatar')} />
          <textarea className={inputClass} placeholder="Short description (shown on your storefront if you're a vendor)" rows={3} value={form.description} onChange={setField('description')} />
        ...
        <div className="mb-10 flex flex-wrap items-center gap-5.5">
          {user.avatar && (
            <img src={user.avatar} alt={user.name} className="h-22.5 w-22.5 rounded-full border-2 border-border-strong object-cover" />
          )}
          <div className="flex-1">
            <div className="mb-1.5 flex flex-wrap items-center gap-3">
              <h1 className="font-display text-[clamp(2.2rem,4vw,3rem)] text-text">{user.name}</h1>
              <button onClick={startEditing} aria-label="Edit profile" className="flex cursor-pointer items-center gap-1.5 border-none bg-none text-[.85rem] text-text-secondary">
                <Pencil size={15} /> Edit
              </button>
            </div>
            <p className="text-text-secondary">{user.email}</p>
            {user.description && <p className="mt-2 max-w-140 leading-[1.6] text-text-secondary">{user.description}</p>}
          </div>
        </div>
      )}
