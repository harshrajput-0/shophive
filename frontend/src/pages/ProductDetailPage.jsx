...
export default function ProductDetailPage() {
  ...
  const handleAdd = () => {
    ...
    dispatch(addItem({ productId: product._id, name: product.name, price: product.price, imageUrl: product.imageUrl, qty: (existing?.qty || 0) + qty }));
  ...
  return (
    <div>
      <div className="mb-6.5 text-[.85rem] text-text-secondary">
        <Link to="/shop" className="text-text-secondary">Shop</Link> / <span className="text-text">{product.name}</span>
      </div>
      ...
      <div className="grid grid-cols-1 gap-12.5 md:grid-cols-2">
        ...
        <div>
          ...
          <h1 className="mb-2.5 font-display text-[clamp(2rem,4vw,2.6rem)] text-text">{product.name}</h1>
          {vendor?.name && (
            <Link to={`/vendor/${vendor._id}`} className="text-[.9rem] font-semibold text-text-secondary">
              Sold by {vendor.name}
            </Link>
          )}
          <p className="my-4.5 font-display text-[2rem] font-bold text-primary">{inr(product.price)}</p>
          <p className="mb-6.5 leading-[1.75] text-text-secondary">{product.description}</p>
          <p className={`mb-6.5 text-[.85rem] font-semibold ${product.stock > 0 ? 'text-success' : 'text-danger'}`}>
            {product.stock > 0 ? `${product.stock} in stock` : 'Currently sold out'}
          </p>
          {product.stock > 0 && (
            <div className="flex flex-wrap items-center gap-5.5">
              <QtyStepper value={qty} onDec={() => setQty((q) => Math.max(1, q - 1))} onInc={() => setQty((q) => Math.min(product.stock, q + 1))} max={product.stock} />
              <Button onClick={handleAdd} className="border-none">Add to Cart</Button>
            </div>
          )}
        </div>
      </div>
