...
const searchInputClass = 'w-full rounded-[10px] border border-border-strong bg-bg-secondary py-3.5 pr-4 pl-11 text-[15px] text-text outline-none';
const selectClass = 'rounded-[10px] border border-border-strong bg-bg-secondary px-4 py-3.5 text-[14px] text-text outline-none';
    ...
    <>
      ...
      <div className="mb-7.5 flex flex-wrap gap-3">
        <div className="relative min-w-55 flex-1">
          ...
          <input className={searchInputClass} type="text" placeholder="Search honey, soap, candles…" value={search} onChange={(e) => setSearch(e.target.value)} />
        </div>
      ...
      <ProductGrid products={visible} loading={status === 'loading' || status === 'idle'} skeletonCount={8} />
    </>
  );
}
