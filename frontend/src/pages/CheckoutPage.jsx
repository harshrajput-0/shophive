    ...
    new Promise((resolve) => {
      paymentService
        ...
        .then((order) => {
          if (!window.Razorpay) {
            dispatch(
              showToast("Payment could not be loaded — check your connection and try again", "err")
            );
            return resolve();
          }
    ...
    <div>
      ...
      <div className="grid grid-cols-1 items-start gap-10 md:grid-cols-[1fr_340px]">
        <form
          ...
          className="flex flex-col gap-4 rounded-[14px] border border-border-strong p-6.5"
        >
        ...
        <div className="rounded-[14px] border border-border-strong p-6.5">
          ...
          <div className="mb-4.5 flex flex-col gap-3">
            {items.map((item) => (
              <div
                key={item.productId}
                className="flex justify-between text-[.9rem] text-text-secondary"
              >
                <span>
                  {item.name} × {item.qty}
                </span>
